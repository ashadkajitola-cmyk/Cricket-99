        ...
        <div id="tabWallet" class="tab-content hidden">
            ...
            <div class="card">
                ...
                <div class="plan-box">
                    ...
                    <button class="action-btn buy-sub-btn" data-cost="99" data-plan="3 Match Subscription" data-matches="3">Buy 3 Match Plan</button>
                </div>
                <div class="plan-box">
...
<script>
    ...
    // CHECK ACTIVE SUBSCRIPTION FOR ADMIN ACCESS
    function checkAdminAccess() {
        ...
        // Check if user has active subscription or is the main admin number
        ...
        if(currentPhone === ADMIN_NUMBER) {
            ...
            if(userData.subExpiry && userData.subExpiry > currentTime) {
                hasActiveSub = true;
            } else if(userData.matchesLeft && userData.matchesLeft > 0) {
                hasActiveSub = true;
            }
        }
    ...
    function buySubscription(cost, planName, extraDays, extraMatches, extraMins) {
        let currentPhone = localStorage.getItem('current_user');
        let users = getUsers();
        let userData = users[currentPhone];

        if(userData.points < cost) {
            alert("Aapke paassufficient points nahi hain!");
            return;
        }
        ...
        if(extraDays) {
            ...
            userData.matchesLeft = extraMatches; // Set 3 matches left explicitly
            userData.subExpiry = 0; // Disable time expiry for match-based plan
        }
    ...
    function updateSubscriptionStatusUI() {
        ...
        if(isActive) {
            if(userData.matchesLeft > 0) {
                subStatusElem.innerText = `Active Plan: ${userData.subscription} (${userData.matchesLeft} matches left)`;
            } else {
    ...
    function saveMatch() {
        ...
        if(editId !== "") {
            ...
            // Naya match schedule karne par agar 3 Match Subscription hai, toh turant expire kar dein
            let currentPhone = localStorage.getItem('current_user');
            if(currentPhone && currentPhone !== ADMIN_NUMBER) {
                let users = getUsers();
                let userData = users[currentPhone];
                if(userData && userData.subscription === "3 Match Subscription" && userData.matchesLeft > 0) {
                    userData.matchesLeft = 0; // Turant matches zero kar do taaki expiration ho jaye
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.buy-sub-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                let matches = parseInt(this.getAttribute('data-matches')) || 0;
                let mins = parseInt(this.getAttribute('data-time')) || 0;
                buySubscription(cost, plan, days, matches, mins);
            });
        });
